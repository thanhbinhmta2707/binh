...
// ===== ADMIN UID =====
// Sau khi login lần đầu, mở Console > gõ: auth.currentUser.uid
// Dán UID vào đây để phân quyền admin
...
function updateNavUI(user) {
  const show = (id, val) => {
    ...
    if (el) el.style.display = val ? (el.classList.contains("user-chip") ? "flex" : "inline-flex") : "none";
  };
  ...
  if (chip) {
    ...
    if (user) {
      ...
      document.getElementById("userChipName").textContent = shortName;
...
async function loadComments(postId) {
  ...
  if (!list) return;
  list.innerHTML = `<div class="comment-empty">⏳ Đang tải...</div>`;
  ...
  try {
    ...
    comments.forEach(c => {
      ...
      item.innerHTML = `
        <div class="comment-item-avatar">${getInitial(c.userName)}</div>
